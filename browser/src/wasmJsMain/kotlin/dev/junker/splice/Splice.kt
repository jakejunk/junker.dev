package dev.junker.splice

import dev.junker.Result
import dev.junker.err
import dev.junker.ok
import dev.junker.splice.cell.SpliceCell
import dev.snipme.highlights.internal.get

class Splice private constructor(
    val sideLength: Int,
    val cells: List<SpliceCell>,
    val operators: Set<PlacedOperator>
) {
    fun applyOperator(
        position: Position,
        operator: SpliceOperator
    ): Result<Splice, String> {
        val candidate = PlacedOperator(position, operator)

        candidate.rhsPosition.toIndex()
            ?: return "Right operand off grid at ${candidate.rhsPosition}".err()

        candidate.resultPosition.toIndex()
            ?: return "Result off grid at ${candidate.resultPosition}".err()

        if (operators.any { it.resultPosition == candidate.resultPosition }) {
            return "Multiple results at ${candidate.resultPosition}".err()
        }

        return Splice(
            sideLength = sideLength,
            cells = cells,
            operators = operators + candidate,
        ).ok()
    }

    fun removeOperator(
        position: Position
    ): Result<Splice, String> {
        val possible = operators
            .mapIndexed { i, placedOp -> i to placedOp }
            .filter { (_, placedOp) -> placedOp contains position }

        if (possible.isEmpty()) {
            return "No operator at $position".err()
        } else if (possible.size > 1) {
            return "Multiple operations at $position".err()
        }

        val (opIndex, _) = possible[0]

        return Splice(
            sideLength = sideLength,
            cells = cells,
            operators = operators - operators[opIndex]!!,
        ).ok()
    }

    fun Int.toPosition(): Position {
        return Position(this % sideLength, this / sideLength)
    }

    fun Position.toIndex(): Int? {
        val i = x + y * sideLength

        return when {
            i >= cells.size -> null
            else -> i
        }
    }

    companion object {
        fun simple(
            sideLength: Int,
            init: (index: Int) -> UByte
        ): Splice {
            return Splice(
                sideLength = sideLength,
                cells = List(sideLength * sideLength, init)
                    .map { value -> SpliceCell(value) },
                operators = emptySet()
            )
        }
    }
}
